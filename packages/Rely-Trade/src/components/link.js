...
const Link = ({ state, href, actions, children, "aria-current": ariaCurrent, Active, rel, className }) => {
    ...
    return (
        <>
           <a href={href} className={className} onClick={onClick} active={Active} aria-current={ariaCurrent} rel={isExternal ? "noopener noreferrer" : rel} onMouseEnter={onMouseEnter}>
            {children}
           </a> 
        </>
    )
